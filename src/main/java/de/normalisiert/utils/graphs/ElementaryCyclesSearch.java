 ...
 * just needs an array of the objects representing the nodes the graph and an
 * adjacency-matrix of type boolean, representing the edges of the graph. It
 * then calculates based on the adjacency-matrix the elementary cycles and
 * returns a list, which contains lists itself with the objects of the concrete
 * graphnodes-implementation. Each of these lists represents an elementary
 * cycle.<br>
 * <br>
 *
...
public class ElementaryCyclesSearch<T> {
   ...
   * @param graphNodes
   *          array of the graphnodes of the graph; this is used to build sets
   *          of the elementary cycles containing the objects of the original
  ...
  /**
   * Returns List::List::Object with the Lists of nodes of all elementary cycles
   * in the graph.
   *
  ...
  public List<List<T>> getElementaryCycles() {
    ...
    Vector<Integer>[] B = new Vector[this.adjList.length];
    this.B = B;
